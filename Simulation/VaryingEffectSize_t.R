...
if (length(args) < 2) {
  stop("Usage: Rscript VaryingEffectSize_t.R <aT> <rep_id> [B=300]")
}
aT      <- as.numeric(args[1])
...
## Fixed design per (lambda0, aT)
seed_X <- as.integer(abs(1e6 * (lambda0 + 10)) + round(aT * 1e6)) %% .Machine$integer.max
set.seed(seed_X)
...
b0 <- -0.1
bG <- 0.1
## aT is now a command-line argument
aW <- rep(0.003, pW)
