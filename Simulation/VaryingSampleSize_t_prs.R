...
if (length(args) < 3) {
  stop("Usage: Rscript VaryingSampleSize_t_prs.R <lambda0> <n> <rep_id> [B=300]")
}
...
C <- cbind(tgt, W)
X <- cbind(G, C)
## coeff[2] now extracts the PRS coefficient
...
objfun_factory <- function(y){
  function(lam){
    ...
    for (k in seq_len(K)) {
      ...
      beta0[k] <- fitk$coeff[2]   # PRS coefficient (1st col of X)
    }
