#!/usr/bin/env bash
#SBATCH --job-name=lam25000_prs_submit
#SBATCH --partition=tier1q
#SBATCH --time=24:00:00
#SBATCH --mem=1G
#SBATCH --cpus-per-task=1
#SBATCH -o logs/lam25000_prs_submit_%A.out
#SBATCH -e logs/lam25000_prs_submit_%A.err
...
RSCRIPT="./VaryingSampleSize_t_prs.R"
OUTDIR="./results_t_prs"
LOGDIR="${OUTDIR}/logs"
RAWDIR="${OUTDIR}/raw"
...
ARRAY_JOBID=$(sbatch --parsable \
  --job-name=lam1rep_prs \
  --partition=tier1q \
  --time=24:00:00 \
  --mem=1G \
  --cpus-per-task=1 \
  -o "${LOGDIR}/lam1rep_prs_%A.out" \
  -e "${LOGDIR}/lam1rep_prs_%A.err" \
...
AGG_JOBID=$(sbatch --parsable \
  ...
  --job-name=lam_agg_prs \
  --partition=tier1q \
  --time=02:00:00 \
  --mem=4G \
  --cpus-per-task=1 \
  -o "${LOGDIR}/lam_agg_prs_%A.out" \
  -e "${LOGDIR}/lam_agg_prs_%A.err" \
  --wrap "Rscript -e '
outdir <- \"${OUTDIR}\"
rawdir <- \"${RAWDIR}\"
lambdas <- c(${LAMBDAS[*]})
