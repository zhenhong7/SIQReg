...
# (with 6-SD outlier exclusion)
# Usage: Rscript lambda_covariate_no_outlier.R <PHENO_ID> <chunk_i> <TARGET_VAR>
...
if (length(args) != 3)
  stop("Usage: Rscript lambda_covariate_no_outlier.R <PHENO_ID> <chunk_i> <TARGET_VAR>")
pheno_id   <- args[1]
...
pheno_path <- Sys.glob(file.path(pheno_dir, paste0(pheno_id, "*.pheno")))
...
mu_pheno <- mean(dat$Pheno)
sd_pheno <- sd(dat$Pheno)
n_before <- nrow(dat)
dat <- dat[abs(Pheno - mu_pheno) <= 6 * sd_pheno]
message(sprintf("[%s] Outlier filter: removed %d / %d samples (>6 SD)",
                pheno_id, n_before - nrow(dat), n_before))
...
out_root <- Sys.getenv("OUTROOT", "results_covariates_no_outlier")
