...
# Read per-phenotype lambda from meta-analysis file
lambda_file <- Sys.getenv("GXE_META_FILE", "results/GxE/meta_both_summary_all.txt")
lambda_table <- read.table(lambda_file, header = TRUE, stringsAsFactors = FALSE)
...
# Look up per-phenotype lambda_WM
lambda_row <- lambda_table[lambda_table$phenotype == my_pheno, ]
if (nrow(lambda_row) == 0) {
    stop(paste("No lambda found for phenotype:", my_pheno))
}
my_lambda <- lambda_row$lambda_DL[1]
print(paste("Lambda_DL being used: ", my_lambda))
...
output_dir <- Sys.getenv("OUTPUT_DIR", "results/GxE")
...
# Run a linear model with the phenotype-specific lambda_WM
# Note: lambdaseq is now a single value per phenotype
descaler <- function( g, e, y, covars, lambdaseq ){
  coefs <- c()
  ps <- c()
  stder <- c()
  r2 <- c()
...
results <- descaler(df$prs, df$env, df$pheno, df[4:ncol(df)], lambdaseq = my_lambda)
