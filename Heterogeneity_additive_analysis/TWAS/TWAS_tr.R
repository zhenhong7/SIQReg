...
if (length(args) < 4) stop("Usage: Rscript TWAS_tr.R <chr> <gene_index> <pheno> <tissue>")
chr        <- args[1]
...
boxcox_transform <- function(y, lambda) if (abs(lambda) < 1e-8) log(y) else (y^lambda - 1)/lambda
...
# age center + quadratic (keep same as original)
...
# transform phenotype (lambda_WM by phenotype)
meta_both_file <- Sys.getenv("META_BOTH_FILE", "Lambda_learning/meta_both_summary_all.txt")
meta_tab <- fread(meta_both_file)

if (!all(c("phenotype", "lambda_WM") %in% names(meta_tab))) {
  stop("meta_both_summary_all.txt must contain columns: phenotype, lambda_WM")
}
...
# original coefs vector for joint/equality tests
...
# bootstrap covariance for joint/equality
n_boot <- 100
...
fwrite(gene_row, file = file.path(out_dir, sprintf("association_results_tr_chr%s.txt", chr)),
       sep = "\t", append = file.exists(file.path(out_dir, sprintf("association_results_tr_chr%s.txt", chr))),
       col.names = !file.exists(file.path(out_dir, sprintf("association_results_tr_chr%s.txt", chr))))
fwrite(age_sex_row, file = file.path(out_dir, sprintf("association_age_sex_tr_chr%s.txt", chr)),
       sep = "\t", append = file.exists(file.path(out_dir, sprintf("association_age_sex_tr_chr%s.txt", chr))),
       col.names = !file.exists(file.path(out_dir, sprintf("association_age_sex_tr_chr%s.txt", chr))))

# NOTE: intentionally NOT saving all_cov_matrices_tr_chr*.RData to avoid parallel load/save corruption
